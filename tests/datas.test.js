...
import { diaAnterior, formatarData, formatarDataCurta } from '../src/utils/datas.ts';
...
describe('formatarDataCurta em TypeScript', () => {
  it('formata string ISO valida no padrao brasileiro', () => {
    expect(formatarDataCurta('2026-05-15')).toBe('15/05/2026');
  });

  it('formata string ISO com hora', () => {
    expect(formatarDataCurta('2026-05-15T12:30:00.000Z')).toBe('15/05/2026');
  });
  ...
  it('retorna traco para string vazia', () => {
    expect(formatarDataCurta('')).toBe('-');
  });

  it('retorna traco para null', () => {
    expect(formatarDataCurta(null)).toBe('-');
  });

  it('retorna traco para undefined', () => {
    expect(formatarDataCurta(undefined)).toBe('-');
  });

  it('formata Date valido', () => {
    expect(formatarDataCurta(new Date('2026-05-15T12:00:00'))).toBe('15/05/2026');
  });

  it('retorna traco para Date invalido', () => {
    expect(formatarDataCurta(new Date('invalida'))).toBe('-');
  });

  it('retorna traco para data invalida (string mal formatada)', () => {
    expect(formatarDataCurta('invalida')).toBe('-');
  });

  it('calcula virada de mes', () => {
    expect(formatarDataCurta('2026-03-01')).toBe('01/03/2026');
  });
