...
describe('prompt manual da IA', () => {
  it('gera prompt aprimorado com formato obrigatorio e dados da questao', () => {
    ...
    expect(prompt).toContain('Você é uma IA especialista em análise de questões de concurso')
  ...
  it('inclui pegadinhas classicas e acao corretiva pratica', () => {
    ...
    expect(prompt).toContain('Palavras absolutas ou restritivas: sempre, nunca, somente')
    expect(prompt).toContain('apenas, todos, nenhum.')
    expect(prompt).toContain('Troca de conceitos parecidos: institutos similares com')
...
describe('prompt de flashcards da questao', () => {
  ...
  it('gera prompt com dados completos da questao e regras principais', () => {
    ...
    expect(prompt).toContain('REGRA DE FORMATAÇÃO MATEMÁTICA — OBRIGATÓRIA')
    expect(prompt).toContain('\\frac{}{}')
    expect(prompt).toContain('PRINCÍPIOS OBRIGATÓRIOS')
    expect(prompt).toContain('Um conceito por card')
    expect(prompt).toContain('recuperação ativa')
    expect(prompt).toContain('Não invente lei, artigo, súmula, jurisprudência')
    expect(prompt).toContain('doutrina ou fundamento que não esteja no material')
    expect(prompt).toContain('Material insuficiente para preencher este campo.')
    expect(prompt).toContain('REGRA DA FRENTE — OBRIGATÓRIA')
    expect(prompt).toContain('Máximo de 20 palavras.')
    expect(prompt).toContain('REGRA DO VERSO — OBRIGATÓRIA')
    expect(prompt).toContain('REGRA DO CONTEXTO — OBRIGATÓRIA')
    expect(prompt).toContain('REGRA DO ALERTA DE BANCA — OBRIGATÓRIA')
    expect(prompt).toContain('CARD DE MEMORIZAÇÃO LITERAL')
  ...
  it('orienta flashcards autocontidos e independentes da questao original', () => {
    ...
    expect(prompt).toContain('sem exigir que')
    expect(prompt).toContain('estudante veja a questão original')
    expect(prompt).toContain('o texto, o enunciado')
    expect(prompt).toContain('Dependência de texto específico')
    expect(prompt).toContain('no trecho sobre X')
    expect(prompt).toContain('nomes ou')
    expect(prompt).toContain('exemplos que só existem')
    expect(prompt).toContain('letras A/B/C/D/E')
    expect(prompt).toContain('alternativa correta')
    expect(prompt).toContain('alternativa marcada')
    expect(prompt).toContain('O comentário e as pegadinhas podem mencionar alternativas')
    expect(prompt).toContain('Ignore as letras')
    expect(prompt).toContain('extraia apenas conceitos')
    expect(prompt).toContain('regras e armadilhas')
    expect(prompt).toContain('transforme-os em cards universais')
    expect(prompt).toContain('Não use referências como "nesta questão"')
    expect(prompt).toContain('conforme o \nenunciado')
    expect(prompt).toContain('Suba um nível de abstração')
    expect(prompt).toContain('regra geral, conceito')
    expect(prompt).toContain('armadilha universal')
    expect(prompt).toContain('O exemplo pode vir da questão trabalhada')
    expect(prompt).toContain('ALERTA DE BANCA deve registrar uma armadilha específica')
    expect(prompt).toContain('reutilizável')
  })
  ...
  it('orienta matematica e raciocinio logico sem comentario pelo metodo especifico', () => {
    ...
    expect(prompt).toContain('QUESTÕES SEM COMENTÁRIO — MATEMÁTICA')
    expect(prompt).toContain('E RACIOCÍNIO LÓGICO')
    expect(prompt).toContain('resolva a questão antes')
    expect(prompt).toContain('de criar os cards')
    expect(prompt).toContain('Identifique o método de resolução correto')
    expect(prompt).toContain('método específico, não apenas')
    expect(prompt).toContain('no tema genérico')
    expect(prompt).toContain('A FRENTE continua universal e autocontida')
    expect(prompt).toContain('passo a passo resumido em notação')
    expect(prompt).toContain('não apenas operações')
  })
